...
from .arxivp import arxivp as arxiv
import pandas as pd
    ...
    try:
        for paper in tqdm(results, desc=f"Processing {query}", disable=not verbose):
            try:
                ...
                # 遇到空页面时跳过，继续处理下一页
                if verbose:
                    logger.warning(f"遇到空页面，跳过并继续处理下一页")
                continue
    except arxiv.UnexpectedEmptyPageError:
        # 如果整个结果集遇到空页面错误，记录警告但继续处理已获取的数据
        if verbose:
            logger.warning(f"查询 '{query}' 遇到空页面错误，但已处理 {len(processed_papers)} 篇论文")

    processed = pd.DataFrame(processed_papers)
    return processed
