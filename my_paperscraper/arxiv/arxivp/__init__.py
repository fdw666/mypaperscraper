...
class Client(object):
    ...
    def __init__(self, page_size: int = 1000, delay_seconds: float = 3.0, num_retries: int = 3):
        """
    ...
    def _results(self, search: Search, offset: int = 0) -> Generator[Result, None, None]:
        ...
        while True:
            ...
            if len(feed.entries)==0:
                self.raise_empty_page_error+=1
            else:
                self.raise_empty_page_error=0
                for entry in feed.entries:
                    try:
                        yield Result._from_feed_entry(entry)
                    except Result.MissingFieldError as e:
                        logger.warning("Skipping partial result: %s", e)
            if self.raise_empty_page_error>2:
                break
            offset += max(len(feed.entries),1)
            if offset >= total_results or offset>=10000:
                break
            page_url = self._format_url(search, offset, self.page_size)
